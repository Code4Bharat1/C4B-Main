...
const FeatureCard = ({ title, description, IconComponent }) => {
  return (
    <div
      className="group relative flex flex-col bg-white shadow-md rounded-lg p-6 md:p-8 w-full h-auto text-left overflow-hidden 
    transition duration-300 transform hover:shadow-lg hover:bg-white hover:-translate-y-2"
    >
      {/* Decorative Circle */}
      <svg
      ...
      <div className="flex-grow">
        ...
        <p className="text-gray-500 text-sm leading-tight md:text-base mb-6 md:mb-8">{description}</p>
      </div>
...
const SolutionsSection = () => {
  const features = [
    ...
    },
    {
      ...
      description:
        "With Odoo Sales, A friendly visual interface to help customize your workflows easily.Planning a multi-stage campaign with various paths is much easier similar to two-step campaigns. Insert new actions and set up time triggers directly on your workflow.you are one click away from converting business quotations into sales orders. You can edit and modify orders and ship orders. Automated invoice generation on ordered and delivered products with details on time, quantities and materials. Easy tracking of order flow with Odoo Sales..",
      IconComponent: FaRegClipboard,
    },
    {
